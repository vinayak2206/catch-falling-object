...
(() => {
      ...
      try {
        ...
        await fetch(`${this.API}/api/leaderboard/submit`, {
          ...
          body: JSON.stringify({
            ...
            avatar:
              (
                CATALOG.baskets.find((b) => s.shop.baskets[b.id]?.equipped) ||
                {}
              ).emoji || "🧺",
            score,
            combo,
        ...
        /* offline is fine — local + seeded board still works */
      }
    },
    async fetchTop() {
      try {
    ...
    play(name) {
      ...
      switch (name) {
        case "coin":
          ...
          setTimeout(() => this.beep(1320, 0.08, "triangle"), 40);
          break;
        case "gem":
          this.beep(1200, 0.06, "sine");
          setTimeout(() => this.beep(1600, 0.06, "sine"), 40);
          setTimeout(() => this.beep(2000, 0.08, "sine"), 80);
          break;
        case "star":
          this.beep(1500, 0.1, "triangle");
          setTimeout(() => this.beep(2000, 0.15, "sine"), 80);
          break;
        case "bomb":
          this.beep(120, 0.25, "sawtooth");
          this.beep(80, 0.3, "square");
          break;
        case "powerup":
          this.beep(500, 0.06, "square");
          setTimeout(() => this.beep(700, 0.06, "square"), 60);
          setTimeout(() => this.beep(1000, 0.12, "square"), 120);
          break;
        case "combo":
          this.beep(800, 0.06, "triangle");
          setTimeout(() => this.beep(1200, 0.06, "triangle"), 40);
          setTimeout(() => this.beep(1600, 0.1, "triangle"), 80);
          break;
        case "click":
          this.beep(600, 0.04, "triangle", 0.5);
          break;
        case "victory":
          [523, 659, 784, 1047].forEach((f, i) =>
            setTimeout(() => this.beep(f, 0.15, "triangle"), i * 100),
          );
          break;
        case "gameover":
          [400, 350, 300, 250].forEach((f, i) =>
            setTimeout(() => this.beep(f, 0.2, "sawtooth"), i * 120),
          );
          break;
      }
    }
  ...
  const WORLDS = [
    {
      name: "Meadow",
      emoji: "🌳",
      sky: ["#4ecdc4", "#95e1d3", "#ffd93d"],
      accent: "#7ee787",
    },
    {
      name: "Beach",
      emoji: "🏖️",
      sky: ["#48cae4", "#ffb347", "#ff8fa3"],
      accent: "#ffd93d",
    },
    {
      name: "Snow Peak",
      emoji: "❄️",
      sky: ["#a8dadc", "#e0f7fa", "#e8f0f7"],
      accent: "#4fc3f7",
    },
    {
      name: "Space",
      emoji: "🚀",
      sky: ["#0d0b26", "#2d1657", "#4a148c"],
      accent: "#9d4edd",
    },
    {
      name: "Candy Land",
      emoji: "🍭",
      sky: ["#ff8fa3", "#ffb3c1", "#ffd93d"],
      accent: "#ff6b6b",
    },
    {
      name: "Volcano",
      emoji: "🌋",
      sky: ["#3d0000", "#ff4757", "#ff9500"],
      accent: "#ff6b35",
    },
  ];
  ...
  function buildLevels() {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      levels.push({
        ...
        bombChance: clamp(0.1 + diff * 0.5, 0.1, 0.6),
  ...
  const ACHIEVEMENTS = [
    {
      id: "first_catch",
      name: "First Catch",
      desc: "Catch your first object",
      goal: 1,
      icon: "🎯",
      check: (s) => s.totalCatches,
    },
    {
      id: "coins_100",
      name: "Coin Collector",
    ...
    },
    {
      id: "perfect_lvl",
      name: "Flawless",
      desc: "Beat a level without damage",
      goal: 1,
      icon: "💎",
      check: (s) => s.perfectLevels,
    },
    {
      id: "level_10",
      name: "Speed Runner",
  ...
  function pickDailyMissions(seedDate) {
    ...
    for (let i = 0; i < seedDate.length; i++)
      seed = (seed * 31 + seedDate.charCodeAt(i)) | 0;
  ...
  class FallingObject {
    ...
    draw(ctx) {
      ...
      const text = this.powerupKind
        ? POWERUP_META[this.powerupKind].icon
        : this.meta.emoji;
  ...
  class Player {
    constructor(game) {
      ...
      this.trailTimer = 0;
      this.emoji =
        CATALOG.baskets.find(
          (b) => SaveManager.data.shop.baskets[b.id]?.equipped,
        )?.emoji || "🧺";
    }
    update(dt, game) {
      ...
      this.trailTimer += dt;
      const trail = CATALOG.trails.find(
        (t) => SaveManager.data.shop.trails[t.id]?.equipped,
      );
            ...
            color:
              trail.id === "fire"
                ? Math.random() < 0.5
                  ...
                  : "#ffd93d"
                : trail.id === "rainbow"
                  ? `hsl(${(Date.now() / 10 + i * 60) % 360},85%,62%)`
                  : trail.id === "bubble"
                    ? "#a0e7e5"
                    : trail.id === "heart"
                      ? "#ff8fa3"
    ...
    catches(obj) {
      ...
      return (
        Math.abs(dx) < this.width / 2 && Math.abs(dy) < this.height / 2 + 10
      );
    }
    refreshSkin() {
      this.emoji =
  ...
  class Background {
    ...
    draw(ctx) {
      ...
      const eqTheme = Object.keys(SaveManager.data.shop.themes).find(
        (k) => SaveManager.data.shop.themes[k]?.equipped,
      );
      const themePal = THEME_PALETTES[eqTheme];
      const world = themePal
        ? { ...worldDefault, sky: themePal.sky, accent: themePal.accent }
        : worldDefault;
  ...
  class Game {
    ...
    setupInput() {
      window.addEventListener("keydown", (e) => {
        if (["ArrowLeft", "a", "A"].includes(e.key)) this.controls.left = true;
          ...
          if (this.running) UI.togglePause();
        }
      });
    ...
    update(dt) {
      ...
      if (this.timeLeft <= 0) {
        this.endLevel(this.score >= this.level.target);
        return;
      }
      ...
      if (this.controls.left)
        this.player.targetX = Math.max(0, this.player.targetX - kbSpeed * dt);
      if (this.controls.right)
        this.player.targetX = Math.min(
          this.width,
          this.player.targetX + kbSpeed * dt,
        );
    ...
    handleCatch(o) {
      ...
      if (o.meta.damage) {
        if (this.activePowerups.SHIELD) {
          ...
          this.particles.burst(o.x, o.y, 34, {
            color: "#ff4757",
            life: 0.9,
            size: rand(4, 12),
          });
      ...
      if (o.meta.powerup) {
        this.activePowerups[o.powerupKind] =
          POWERUP_META[o.powerupKind].duration;
        this.floats.add(
          o.x,
          o.y - 20,
          POWERUP_META[o.powerupKind].name,
          POWERUP_META[o.powerupKind].color,
          22,
        );
        this.particles.burst(o.x, o.y, 24, {
          color: POWERUP_META[o.powerupKind].color,
          life: 0.9,
        });
        audio.play("powerup");
        UI.toast(
          `${POWERUP_META[o.powerupKind].icon} ${POWERUP_META[o.powerupKind].name}!`,
          "success",
        );
    ...
    endLevel(won) {
      ...
      if (won) {
        ...
        s.levelStars[this.level.id] = Math.max(
          s.levelStars[this.level.id] || 0,
          stars,
        );
        s.stars += stars;
      ...
      if (won) UI.showVictory(this);
      else UI.showGameOver(this);
    }
    checkAchievements() {
      const s = SaveManager.data;
  ...
  const UI = {
    ...
    init() {
      ...
      document.querySelectorAll(".tab").forEach((t) => {
        t.addEventListener("click", () => {
          ...
          if (shopTab) {
            document
              .querySelectorAll(".tab[data-shop-tab]")
              .forEach((x) => x.classList.remove("active"));
            t.classList.add("active");
            this.renderShop(shopTab);
          } else if (lbTab) {
            document
              .querySelectorAll(".tab[data-lb-tab]")
              .forEach((x) => x.classList.remove("active"));
            t.classList.add("active");
      ...
      document.getElementById("name-save").addEventListener("click", () => {
        const name =
          document.getElementById("name-input").value.trim() || "Player";
      ...
      // Daily
      document
        .getElementById("claim-daily")
        .addEventListener("click", () => this.claimDaily());
      document
      ...
      switch (action) {
        ...
        case "next-level":
          ...
          if (nx < LEVELS.length) this.game.startLevel(nx);
            ...
            UI.toast("🏆 You beat all levels!", "success");
            this.showScreen("menu");
            this.updateMenu();
          }
          break;
    ...
    togglePause() {
      ...
      if (this.game.paused) this.showOverlay("pause");
      else this.hideOverlay("pause");
    },
    updateMenu() {
      const s = SaveManager.data;
      ...
      const eqBasket = CATALOG.baskets.find(
        (b) => s.shop.baskets[b.id]?.equipped,
      );
    ...
    updateHUD(g) {
      ...
      hEl.innerHTML = "";
      for (let i = 0; i < 3; i++)
        hEl.innerHTML += `<span>${i < g.hearts ? "❤️" : "🤍"}</span>`;
      document.getElementById("hud-coins").textContent = g.coinsEarned;
      document.getElementById("hud-score").textContent = g.score;
      document.getElementById("hud-level-label").textContent =
        g.level.name.toUpperCase();
    ...
    renderLevels() {
      ...
      for (let w = 0; w < WORLDS.length; w++) {
        ...
        const prevWorldLast =
          w > 0 ? LEVELS.filter((l) => l.world === w - 1).slice(-1)[0] : null;
        const worldLocked =
          prevWorldLast && !SaveManager.data.levelStars[prevWorldLast.id];
        const cleared = worldLevels.filter(
          (l) => SaveManager.data.levelStars[l.id],
        ).length;
      ...
      `;
        ...
        for (const lvl of worldLevels) {
          ...
          const locked =
            worldLocked ||
            (lvl.id > 1 && !SaveManager.data.levelStars[prev.id]);
    ...
    renderShop(tab) {
      ...
      for (const it of items) {
        ...
        const bg = {
          common:
            "linear-gradient(135deg, rgba(176,190,197,0.2), rgba(255,255,255,0.05))",
          ...
          legendary:
            "linear-gradient(135deg, rgba(255,217,61,0.3), rgba(255,107,107,0.15))",
        }[it.rarity];
        card.innerHTML = `
      ...
      `;
        card.addEventListener("click", () => {
          audio.play("click");
          if (!owned) {
            if (SaveManager.data.coins < it.price)
              return UI.toast("Not enough coins!", "error");
            SaveManager.data.coins -= it.price;
            SaveManager.data.shop[tab][it.id] = {
              owned: true,
              equipped: false,
            };
            SaveManager.save();
            ...
            for (const k of Object.keys(SaveManager.data.shop[tab])) {
              if (SaveManager.data.shop[tab][k])
                SaveManager.data.shop[tab][k].equipped = false;
            }
    ...
    renderMissions() {
      ...
      const m = SaveManager.data.missions;
      document.getElementById("missions-refresh").textContent =
        Missions.timeUntilRefresh();
    ...
    claimDaily() {
      ...
      if (s.daily.lastClaim === today)
        return UI.toast("Already claimed today!", "warn");
      const rewards = [10, 20, 30, 50, 75, 100, 200];
    ...
    spinWheel() {
      const s = SaveManager.data;
      const today = new Date().toDateString();
      if (s.daily.spinDate === today)
        return UI.toast("Already spun today!", "warn");
  ...
  function boot() {
    ...
    if (SaveManager.data.settings.reducedMotion)
    ...
    const iv = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setTimeout(() => {
          UI.showScreen("menu");
          if (SaveManager.data.firstRun || !SaveManager.data.player)
            UI.showOverlay("name-prompt");
        }, 150);
      }
    }, 90);
